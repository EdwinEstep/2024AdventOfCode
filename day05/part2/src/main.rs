...
fn main() -> std::io::Result<()> {
    ...
    for update in &updates {
        ...
        if !good {
            while !fixed {
                for rule in &local_rules {
                    if !passes_rule_check(rule, &new_update) {
                        new_update = check_and_fix(rule, &new_update);
            ...
            mid_page_nums += new_update[update.len()/2];
            }
        }
    }
...
fn check_and_fix(rule: &Vec<u32>, update: &Vec<u32>) -> Vec<u32> {
